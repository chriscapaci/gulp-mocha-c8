...
module.exports = opts => {
    ...
    for (const key of Object.keys(mochaOpts)) {
        ...
        if (Array.isArray(val)) {
            if (!MULTIPLE_OPTS_MOCHA.has(key)) {
                // Convert arrays into space separated lists
                mochaOpts[key] = val.join(' ');
            }
        } else if (typeof val === 'object') {
            // Convert an object into comma separated list
            mochaOpts[key] = convertObjectToList(val);
        }
    }
    ...
    function flush(done) {
        ...
        const proc = execa('c8 ' + c8Args.concat(['mocha'].concat(mochaArgs.concat(files))).join(' '), {
            env,
