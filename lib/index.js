...
module.exports = opts => {
    ...
    for (const key of Object.keys(mochaOpts)) {
        ...
        if (Array.isArray(val)) {
            ...
            mochaOpts[key] = utils.convertObjectToList(val);
        }
    }
    ...
    for (const key of Object.keys(c8Opts)) {
        ...
        if (Array.isArray(val)) {
            ...
            // Convert an object into comma separated list
            c8Opts[key] = utils.convertObjectToList(val);
        }
    }
    ...
    function flush(done) {
        ...
        const proc = execa('c8', c8Args.concat(['mocha'].concat(files.concat(mochaArgs))), {
            env,
